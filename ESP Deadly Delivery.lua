...
--   Version : 7.1.0 (Perbaikan Pemicu Tombol E)
-- ================================================================
...
local function getSafeZoneGround()
    local platform = getPath({"电梯", "Left4", "升降台"})
    return platform and platform:FindFirstChild("Ground") or nil
end

local function getFoodGathering()
    return getPath({"电梯", "FoodGathering"})
end

local function getPlayerLootsFolder()
    return getPath({"GameSystem", "Loots", "Player"})
end

local function isItemSafeInBase(loot)
    local ground = getSafeZoneGround()
    if not ground then return false end
    if CONFIG.Blacklist[loot.Name] then
        local distance = (loot:GetPivot().Position - ground.Position).Magnitude
        if distance <= CONFIG.SafeZoneRadius then
            return true 
        end
    end
    return false
...
local function dropItemInFolder()
    local playerFolder = getPlayerLootsFolder()
    if not playerFolder then return false end
    
    local items = playerFolder:GetChildren()
    for _, item in ipairs(items) do
        if not ignoredMonsters[item] then
            pressKey(Enum.KeyCode.G)
            task.wait(0.5)
            
            if item.Parent == playerFolder then
                ignoredMonsters[item] = true
            end
            return true
        end
    end
...
local function verifyAndEmptyBackpack(hrp, foodGathering)
    hrp.CFrame = foodGathering:GetPivot()
    task.wait(2)
...
local function handleWalkSpeedDrop(hrp, humanoid, foodGathering)
...
local function triggerAutoCollect()
    ...
    local foodGathering = getFoodGathering()
    
    if not hrp or not humanoid or not foodGathering then 
